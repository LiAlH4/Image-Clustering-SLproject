...
def get_model(p, pretrain_path=None):
    # Get backbone
    if p['backbone'] == 'resnet18':
        if p['train_db_name'] in ['cifar-10', 'cifar-20','mnist','fashion-mnist']:
...
def get_train_dataset(p, transform, to_augmented_dataset=False,
    ...
    if p['train_db_name'] == 'cifar-10':
        ...
        dataset = FASHION_MNIST(train=True, transform=transform, download=True)
...
def get_val_dataset(p, transform=None, to_neighbors_dataset=False):
    ...
    if p['val_db_name'] == 'cifar-10':
        ...
        from data.fashion_mnist import FASHION_MNIST
        dataset = FASHION_MNIST(train=False, transform=transform, download=True)
